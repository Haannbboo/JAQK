...
def setup():
    ...
    with open(datapath(False, 'setup_cache.txt'), mode='w') as w:
    ...
    dirs = [_os.listdir(datapath(True, c, setup=True)) for c in companies]
    dirs2 = dirs[:]
    del dirs
    try:
    ...
    if '.py' in ''.join(dirs2[0]) + ''.join(dirs2[1]):  # AAPL and AMZN
        ...
        [_os.remove(datapath(True, companies[i], dirs2[i][j], setup=True))
    ...
    dirs_general2 = _os.listdir(datapath(True, 'general', setup=True))
    ...
    if '.py' in ''.join(dirs_general):  # NYSE and NASDAQ
        ...
        [open_general(ex, setup=True).to_csv(_os.path.join(setup_path, 'general', ex + '.csv'), index=False)
         for ex in exc]
        [_os.remove(datapath(True, 'general', ex + '.py', setup=True)) for ex in exc]
